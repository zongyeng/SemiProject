...
public class Broadsocket {
	...
	private static UserinfoDto dto = null;
	...
	private boolean noConsultantRoom(Session session, UserinfoDto dto) {
	...
	public UserinfoDto createdto(Session session) {
	...
	public void addNickname(Session session, UserinfoDto dto) {
		nicknames.put(session, dto.getUserinfo_nicname());
	}

	// 자료의 직접적인 관리는 chatting.jsp에서 실시합니다.
	public void sendUserinfoForm(UserinfoDto dto) throws IOException {
		Consultant.getBasicRemote()
				.sendText("(SyStEm)USER[SEQ=" + dto.getUserinfo_seq() + ",NAME=" + dto.getUserinfo_name() + ",NICKNAME="
						+ dto.getUserinfo_nicname() + ",SEX=" + dto.getUserinfo_sex() + ",PHON="
						+ dto.getUserinfo_phonnumber() + ",EMAIL=" + dto.getUserinfo_email() + "]");
	}

	public boolean overlapNickname(Session session, UserinfoDto dto) {
		for (Session client : clients) {
			try {
				if (nicknames.get(client).equals(dto.getUserinfo_nicname())) {
